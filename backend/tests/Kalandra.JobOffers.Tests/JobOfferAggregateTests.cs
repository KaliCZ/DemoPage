...
public class JobOfferAggregateTests
{
    ...
    public void Edit_ByOwner_WhenSubmitted_Succeeds()
    {
        ...
        var result = offer.Edit(
            user: Owner,
            companyName: NE("NewCo"),
            contactName: NE("Jane"),
            contactEmail: NE("jane@co.com"),
            jobTitle: NE("CTO"),
            description: NE("New desc"),
            salaryRange: null,
            location: null,
            isRemote: true,
            additionalNotes: null,
        ...
        Assert.Equal("NewCo", evt.CompanyName?.Value);
    }
    ...
    public void Edit_PartialUpdate_OnlyChangesProvidedFields()
    {
        ...
        Assert.Equal("CTO", evt.JobTitle?.Value);
    ...
    public void Edit_ByNonOwner_Fails()
    {
        ...
        Assert.Equal(EditJobOfferError.NotAuthorized, result.Error!);
    }
    ...
    public void Edit_WhenNotSubmitted_Fails()
    {
        ...
        offer.Apply(new JobOfferCancelled(
            ...
            CancelledByEmail: NE("owner@test.com"),
            Reason: null,
            Timestamp: Now));
        ...
        Assert.True(result.IsError);
        Assert.Equal(EditJobOfferError.NotSubmittedStatus, result.Error!);
    }
    ...
    public void Cancel_ByNonOwner_Fails()
    {
        ...
        Assert.True(result.IsError);
        Assert.Equal(CancelJobOfferError.NotAuthorized, result.Error!);
    }
    ...
    public void Cancel_WhenLetsTalk_Fails()
    {
        ...
        var result = offer.Cancel(user: Owner, reason: null, timestamp: Now);
        Assert.True(result.IsError);
        Assert.Equal(CancelJobOfferError.InvalidStatus, result.Error!);
    }
    ...
    public void Apply_Submitted_SetsInitialState()
    {
        ...
        offer.Apply(new JobOfferSubmitted(
            UserId: u1,
            UserEmail: NE("u1@test.com"),
            CompanyName: NE("Co"),
            ContactName: NE("Name"),
            ContactEmail: NE("c@co.com"),
            JobTitle: NE("Dev"),
            Description: NE("Desc"),
            SalaryRange: "$100k",
            Location: "NYC",
    ...
    public void Apply_Edited_NullFields_ArePreserved()
    {
        ...
        offer.Apply(new JobOfferEdited(
            ...
            ContactName: null,
            ContactEmail: null,
            JobTitle: NE("CTO"), // the only field actually edited
            Description: null,
            SalaryRange: null,
            Location: null,
            IsRemote: null,
    ...
    public void Edit_OnlyEmitsChangedFields()
    {
        ...
        var result = offer.Edit(
            user: Owner,
            companyName: NE("Acme"),           // unchanged
            contactName: NE("John"),           // unchanged
            contactEmail: NE("john@acme.com"), // unchanged
            jobTitle: NE("Senior Dev"),        // changed (was "Dev")
            description: NE("Desc"),           // unchanged
        ...
        Assert.Null(evt.ContactName);
        Assert.Null(evt.ContactEmail);
        Assert.Equal("Senior Dev", evt.JobTitle?.Value);
        Assert.Null(evt.Description);
        Assert.Null(evt.SalaryRange);
        Assert.Null(evt.Location);
        Assert.Null(evt.IsRemote);
